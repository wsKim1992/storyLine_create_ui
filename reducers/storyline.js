    ...
    sampleData:{
        createdStory:[
            ...
            },
            {
                id:12345678,
                inputType:'text'
                ,outputText:'한동안 방황해 온 현정과 가람은 피곤해 길을 잃는다. 그들은 아무것도 먹지 않았기 때문에 배가 고팠고, 낯선 사람들을 두려워했기 때문에 매우 고통스러웠다. 어두운 골목에 숨어 있던 현정과 가람은 쓰레기통에서 발견한 빵 등 물건들을 먹으며 낯선 사람들을 무서워해 달아났다. 그들은 도시를 돌아다녔고, 낯선 사람들로부터 도망치고 있었다. 그러다 현정과 가람은 이상한 모자를 쓰고 있던 낯선 남자를 보았다. 그는 마술사였다. '
            },
            {
                id:14321234,
                inputType:'image'
    ...
    return produce(state,(draft)=>{
        switch (action.type){
            ...
            case LOAD_SAMPLE_DATA:{
                draft.createdStory=draft.sampleData.createdStory;
                draft.creatingStory=draft.sampleData.creatingStory;
                break;
            }
            ...
            case LOAD_STORY_REQUEST:{
                ...
                if(draft.creatingStory){
                    if(draft.creatingStory.inputType==='text'){
                        draft.createdStory=draft.createdStory.concat({
                            id:draft.creatingStory.id,
                            inputType:draft.creatingStory.inputType,
                            outputText:draft.creatingStory.inputText?draft.creatingStory.inputText.concat(draft.creatingStory.outputText[draft.creatingStory.index])
                                :draft.creatingStory.outputText[draft.creatingStory.index]
                        })
                    }else{
                        draft.createdStory=draft.createdStory.concat({
                            id:draft.creatingStory.id,
